...
function removeTask(e) {
    ...
    if (e.target.parentElement.classList.contains('delete-item')) {
        if (confirm('是否确认完成？')) {
            ...
            //删除本地存储任务
            removeTaskFormLoaclStorage(e.target.parentElement.parentElement);
        }
    }
...
function storeTaskInLocalStorage(task) {
    ...
    // 将传入的task添加到tasks中
    tasks.push(task);
    // 进行本地存储，设置键名为tasks，因为传入的值得是string所以用json的方法将数组转为string
...
// 从本地存储中删除目标任务
function removeTaskFormLoaclStorage(taskItem) {
